...
fileID = fopen('combined_model_5_wKNN.txt','w');
...
%     cutoff_R_tfbs = sum(PC4_R_tfbs(PC4_R_tfbs>0 & PC4_R_tfbs<Inf))/sum(PC4_R_tfbs>0 & PC4_R_tfbs<Inf); % AVERAGE OF POSITIVE VALUES (Prone)
    ...
    %TRAIN = horzcat(K2_3,K3_3,K4_3,K5_3,K6_3,K7_3,K8_3);
    ...
    %TEST = horzcat(newX2,newX3,newX4,newX5,newX6,newX7,newX8);     
...
%     CLASSIFIER = fitcsvm(TRAIN, labels_train_logical,'Standardize',1,'KernelFunction','rbf','KernelScale','auto'); 
     CLASSIFIER = fitcknn(TRAIN, labels_train_logical, 'NumNeighbors',9,'Distance','euclidean','Standardize',1);    
