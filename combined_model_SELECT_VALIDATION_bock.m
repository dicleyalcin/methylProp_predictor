...
deleteSeq_P = {'5C18','5C36','4B44','1D06','6C12','2C19','2E26','3D65','2E30','3D70','3C70','4B56','5F51','3F62','4F07'};
deleteSeq_R = {'4C23','5G64','5G64_SRRD','1C4','5C6','2D67','2C12','3D44','2C13','3B35','2F79','3E3'};


[sp, isp]=setdiff(k_mer_seqheader_prone_feltus, deleteSeq_P, 'stable');
[sr, isr]=setdiff(k_mer_seqheader_resistant_feltus, deleteSeq_R, 'stable');



datak2_p = datak2(1:28,:); 
datak2_r = datak2(29:75,:);
datak2_p_s = datak2_p(isp,:);
datak2_r_s = datak2_r(isr,:);
feltusdatak2 = [datak2_p_s; datak2_r_s];

datak3_p = datak3(1:28,:); 
datak3_r = datak3(29:75,:);
datak3_p_s = datak3_p(isp,:);
datak3_r_s = datak3_r(isr,:);
feltusdatak3 = [datak3_p_s; datak3_r_s];

datak4_p = datak4(1:28,:); 
datak4_r = datak4(29:75,:);
datak4_p_s = datak4_p(isp,:);
datak4_r_s = datak4_r(isr,:);
feltusdatak4 = [datak4_p_s; datak4_r_s];

datak5_p = datak5(1:28,:); 
datak5_r = datak5(29:75,:);
datak5_p_s = datak5_p(isp,:);
datak5_r_s = datak5_r(isr,:);
feltusdatak5 = [datak5_p_s; datak5_r_s];

datak6_p = datak6(1:28,:); datak6_r = datak6(29:75,:);
datak6_p_s = datak6_p(isp,:);datak6_r_s = datak6_r(isr,:);
feltusdatak6 = [datak6_p_s; datak6_r_s];

datak7_p = datak7(1:28,:); datak7_r = datak7(29:75,:);
datak7_p_s = datak7_p(isp,:);datak7_r_s = datak7_r(isr,:);
feltusdatak7 = [datak7_p_s; datak7_r_s];

datak8_p = datak8(1:28,:); datak8_r = datak8(29:75,:);
datak8_p_s = datak8_p(isp,:);datak8_r_s = datak8_r(isr,:);
feltusdatak8 = [datak8_p_s; datak8_r_s];

datak9_p = dataK9(1:28,:); datak9_r = dataK9(29:75,:);
datak9_p_s = datak9_p(isp,:);datak9_r_s = datak9_r(isr,:);
feltusdatak9 = [datak9_p_s; datak9_r_s];



load('KMER_BOCK.mat');
load('seq_header_column_BOCK.mat');

for i=1:length(seqheadersbock)
    seqheadersbock{i} = strcat('S',num2str(seqheadersbock{i}));
end

datas_kmer_bock = {bockdatak2, bockdatak3, bockdatak4, bockdatak5, bockdatak6, bockdatak7, bockdatak8, bockdatak9};        


k_mer_seqheader_prone_bock = seqheadersbock(1:np_v,:);        
k_mer_seqheader_resistant_bock = seqheadersbock((np_v+1):n_v,:);



filename = sprintf('Score2_MEME_TRAIN_FELTUS_select');
...
motif_seqheader_prone = PM(2:(np_s+1),1); 
motif_seqheader_resistant = PM((np_s+2):(np_s+nr_s+1),1);


% Significant motifs 
filememe = sprintf('SigMotif_FELTUS_select');
...
PM_prone = PM(2:(np_s+1),:); 
RM_prone = RM(2:(np_s+1),:); 

PM_resistant = PM((np_s+2):(np_s+nr_s+1),:); 
RM_resistant = RM((np_s+2):(np_s+nr_s+1),:); 
...
%% select 
[cp, iap, ibp]=intersect(sp,motif_seqheader_prone,'stable');
[cr, iar, ibr]=intersect(sr,motif_seqheader_resistant,'stable'); 
...
numericCells_P = PM(1:(np_s+nr_s),2:31); 
numericCells_R = RM(1:(np_s+nr_s),2:31); 

DATA_P = cell2mat(numericCells_P);
DATA_R = cell2mat(numericCells_R);

xP_P = DATA_P(1:np_s,:); xP_R = DATA_P((np_s+1):(np_s+nr_s),:);
xR_P = DATA_R(1:np_s,:); xR_R = DATA_R((np_s+1):(np_s+nr_s),:); 
...
%%SELECT
TRAIN_DATAK2 = feltusdatak2;
TRAIN_DATAK3 = feltusdatak3;
TRAIN_DATAK4 = feltusdatak4;
TRAIN_DATAK5 = feltusdatak5;
TRAIN_DATAK6 = feltusdatak6;
TRAIN_DATAK7 = feltusdatak7;
TRAIN_DATAK8 = feltusdatak8;
TRAIN_DATAK9 = feltusdatak9;
...
[sp_dfam, isp_dfam] = setdiff(dfam_seqheader_prone, deleteSeq_P, 'stable');
[sr_dfam, isr_dfam] = setdiff(dfam_seqheader_resistant,deleteSeq_R,'stable');


dfamdata = consdata';

consdata_p = dfamdata(1:28,:);
consdata_r = dfamdata(29:75,:);

d_prone = consdata_p(isp_dfam,:);
d_resistant = consdata_r(isr_dfam,:);

dfamdata = [d_prone; d_resistant];

dataD_prone = dfamdata(1:np_s,:);        
dataD_resistant = dfamdata((np_s+1):n_s,:); 

dfam_seqheader_prone_select = sp_dfam;
dfam_seqheader_resistant_select = sr_dfam;

[Cdp,id_p,ie_p] = intersect(dfam_seqheader_prone_select,motif_seqheader_prone,'stable');
[Cdr,id_r,ie_r] = intersect(dfam_seqheader_resistant_select,motif_seqheader_resistant,'stable');
...
%% TFBS_FELTUS (TRAIN)
filename_tfbs = sprintf('Score2_TFBS_TRAIN_FELTUS_select');
...
raw_PM_tfbs = raw_PM_tfbs(2:(np_s+1),:);
    
    
raw_RM_tfbs = raw_dataR_tfbs';
raw_RM_tfbs = raw_RM_tfbs(2:(nr_s+1),:);
    
tfbs_motif_seqheader_prone = raw_PM_tfbs(1:np_s,1);        
tfbs_motif_seqheader_resistant = raw_RM_tfbs(1:nr_s,1);
...
PM_tfbs = dataP_tfbs;
RM_tfbs = dataR_tfbs;
numericCells_P_tfbs = PM_tfbs(1:np_s,2:483);
numericCells_R_tfbs = RM_tfbs(1:nr_s,2:483);   
...
labels_train_tfbs = num2str([zeros(np_s,1) ; ones(nr_s,1)]);
...
filename_test = sprintf('Score2_MEME_BOCK');
...
[cptest, iaptest, ibptest]= intersect(k_mer_seqheader_prone_bock, motif_seqheader_prone_test, 'stable');        
[crtest, iartest, ibrtest]= intersect(k_mer_seqheader_resistant_bock, motif_seqheader_resistant_test,'stable');     
...
% test K-MER data (Bock validation)    
%% K = 2
test_datak2_prone = bockdatak2(1:np_v,:); 
test_datak2_resistant = bockdatak2((np_v+1):n_v,:);        
%% K = 3
test_datak3_prone = bockdatak3(1:np_v,:); 
test_datak3_resistant = bockdatak3((np_v+1):n_v,:);
%% K = 4
test_datak4_prone = bockdatak4(1:np_v,:); 
test_datak4_resistant = bockdatak4((np_v+1):n_v,:);
%% K = 5
test_datak5_prone = bockdatak5(1:np_v,:); 
test_datak5_resistant = bockdatak5((np_v+1):n_v,:);
%% K = 6
test_datak6_prone = bockdatak6(1:np_v,:); 
test_datak6_resistant = bockdatak6((np_v+1):n_v,:);
%% K = 7
test_datak7_prone = bockdatak7(1:np_v,:); 
test_datak7_resistant = bockdatak7((np_v+1):n_v,:);
%% K = 8
test_datak8_prone = bockdatak8(1:np_v,:); 
test_datak8_resistant = bockdatak8((np_v+1):n_v,:);
%% K = 9
test_datak9_prone = bockdatak9(1:np_v,:); 
test_datak9_resistant = bockdatak9((np_v+1):n_v,:);
...
load('DFAM_BOCK.mat');
dfam_seqheader_prone_bock = seqheadersbock(1:np_v,:);
dfam_seqheader_resistant_bock = seqheadersbock((np_v+1):n_v,:);
dfamdata_bock = consdata_bock';
dataD_prone_bock = dfamdata_bock(1:np_v,:);
dataD_resistant_bock = dfamdata_bock((np_v+1):n_v,:);

[Cdp_test,id_p_test,ie_p_test] = intersect(dfam_seqheader_prone_bock,motif_seqheader_prone_test,'stable');
[Cdr_test,id_r_test,ie_r_test] = intersect(dfam_seqheader_resistant_bock,motif_seqheader_resistant_test,'stable');                

test_dfam_prone = dataD_prone_bock(id_p_test,:);
test_dfam_resistant = dataD_resistant_bock(id_r_test,:);   

%%%%%%%%%%%%%%%%%%
TEST_DFAM = [test_dfam_prone; test_dfam_resistant];
%%%%%%%%%%%%%%%%%%

filename_test_tfbs = sprintf('Score2_TFBS_BOCK');
...
[Cp_tfbs_test,ia_p_tfbs_test,ib_p_tfbs_test] = intersect(k_mer_seqheader_prone_bock ,tfbs_motif_seqheader_prone_test,'stable');
[Cr_tfbs_test,ia_r_tfbs_test,ib_r_tfbs_test] = intersect(k_mer_seqheader_resistant_bock ,tfbs_motif_seqheader_resistant_test,'stable');
...
labels_train = num2str([zeros(np_s,1) ; ones(nr_s,1)]);
...
TRAIN = horzcat(K2_3,K3_3,K4_3,K5_3,K6_3,K7_3,K8_3,TRAIN_MOTIF,TRAIN_DFAM);
...
% TEST = horzcat(newX2,newX3,newX4,newX5,newX6,newX7,newX8,TEST_MOTIF,TEST_TFBS,TEST_DFAM);   
 TEST = horzcat(newX2,newX3,newX4,newX5,newX6,newX7,newX8,TEST_MOTIF,TEST_DFAM);   
%%%%%%%%%%%%%%%
